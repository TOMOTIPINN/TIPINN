...
| 同意するまで非表示 | 顧客側4画面と購入導線に手が入る。必要性は下記の未確認事項次第 |

#### 未確認

1. **店長の登録から本人の受諾までの「同意前に表示される期間」**が問題ないか。
   山口先生に確認予定。問題であれば非表示を実装する。
2. **本人の受諾フローの実機確認は未実施。** 未紐付けの LINE アカウントが用意できないため。
   次に新しいスタッフが参加したときに SQL で記録を確認する。
3. サロン向け規約・契約書への説明義務の記載。
