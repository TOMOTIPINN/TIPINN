...
### 10.1 同意の取得はサロンの責任とし、店長が申告する（2026-09-15 決定）

#### 背景

スタッフの氏名・写真は、店長が `/manager/staff` で追加した瞬間から顧客に表示される。
`staff` 行の公開条件は `salon_id` 一致 ＋ `archived_at is null` だけで、
本人の LINE 紐付けも、本人が echo の画面に到達することも不要
＝**本人が知らないまま公開され得る**。
...
1. 同意の取得は**サロンの責任**とする。店長がスタッフ追加時に
   「本人に、氏名・写真などがお客様に表示されることを説明し、同意を得ています」を
   チェックする。**本人によるチェックは取らない。**
2. 保存するのは**日時**（`publish_consent_confirmed_at`）と
   **申告した店長の `staff.id`**（`publish_consent_confirmed_by`）。
   `confirmed_by` はクライアントから受け取らず、`requireManager()` が解決した
   `ctx.staff_id` を使う。
3. **公開の可否は制御しない。** この2列は記録であって制御ではない。顧客側の表示条件は
   従来どおり `archived_at is null` のみで、0045 でも変えていない。
   ここを公開ゲートにすると、既存の全スタッフ（`confirmed_at` が null）が
   一斉に非表示になり、稼働中サロンの感想フォームからスタッフが消える。
4. 検証は**サーバー側でも行う**。未チェック・欠落は 400（`consent_required`）。
   フォームのボタン disabled は UI の補助にすぎない。
5. オーナー自動登録（`/api/manager/salon/new`）は**対象外**。両列は null のまま。
   自分で自分を登録する経路には、説明も同意取得も行っていないため
   （記録に日時を入れると実態より強い主張になる）。
6. 既存スタッフは口頭で同意を取り、取得日を SQL で入れる。**未実施。**

実装 commit `d228224`。migration 0045 は 2026-09-15 に SQL エディタで手動適用済み。

#### 採らなかった案と理由

| 案 | 採らなかった理由 |
|---|---|
| 本人ログイン時の同意ゲート | 既存スタッフは口頭で取れるため不要。`/staff` `/manager` の layout はセッションを読まない pass-through で、認証は14ページが個別に持つ。組み込みに手が広く実装が重い |
| 招待受諾時（`/staff/join`）の本人チェック | 受諾より**前**に公開が始まる順序は変わらない。本人が受諾しないまま公開され続ける穴が残る |
| 同意しない場合に非表示 | 顧客側4画面と購入導線に手が入る。拒否者が出たらアーカイブ運用（`archived_at`）で受ける |

#### 未確認

1. 店長の申告方式で許諾取得として足りるか（山口先生に質問予定）。
2. サロン向け規約への「同意取得はサロンの責任」の記載。
