...
4. **1感想につき有料スタンプは1回まで。**
   `rating_purchases` に `review_id` の**部分一意制約**（`review_id is not null`）を想定。
   既存21件は全件 null のため衝突しない想定（**本番未確認**）。
...
1. 購入画面に「お名前がスタッフに表示されます」の一文を出すか、
   規約・プライバシーポリシー記載で足りるか（山口先生に確認）。
2. webhook の実到着時間・再送頻度は**未確認**。
3. 部分一意制約の既存データとの衝突有無は、migration 作成時に**本番で確認**する。
4. docs 上の「感想だけ送る」導線が現 `RatingPicker` に存在しない（別件・記録のみ）。
5. **実装は未着手。**
