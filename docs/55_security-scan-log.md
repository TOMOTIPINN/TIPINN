...
> なので **`docs/55_security-scan-log.md`** へ移した（`50_security.md` の隣＝連番の並びに載せる）。
> **過去のエントリは書き換えない**という方針（§6）は変わらない。
...
**月次レビュー日: 毎月25日。** 当日に上記4件を確認し、本ファイルへ結果を追記する。
Dependabot・CodeQL は自動検知のため、緊急度の高い指摘は月次を待たず随時対応する。

> **2026-09-23 変更**: レビュー日を**「毎月第1日曜」から「毎月25日」へ**改めた。
> 理由は**運用の実態に合わせるため**（第1日曜では実施できておらず、§3 のとおり
> 2026-09-06 予定分の記録が残っていない）。
...
## 4. 2026-09-23 第2回 定期診断

- 実施日: 2026-09-23（水）
- 実施者: 原 朋之
- 診断時点のコミット: `44e5932` → 対応後 `f0040ad`
- §3 のとおり 2026-09-06 予定分の記録は無く、**本ファイルの実施記録としてはこれが第2回**。
...
§6「未解消の指摘は『未解消』として残す」に従い、**この回で閉じなかったもの**を明示する。

**(1) アクセス制御の棚卸しが古い。**
`docs/access-control-audit.md` は **2026-08-02・全25ページ**の記録のまま更新されていない。
2026-09-23 時点で `src/app/**/page.tsx` は **33枚**（`/owner` 系3枚を含む）＝**8枚ぶん未監査**。
**棚卸しの再実施は未対応。**

**(2) 認可ガードを各ページに個別実装している構造と、§21 の決定との関係が未整理。**
§2-7 は「ガードが layout ではなく各ページに個別実装されているため、新規ページを追加した際に
実装漏れが発生し得る」を**構造的リスク**として挙げ、「**layout への集約を次回以降に検討する**」
としていた。
一方 `/owner` の実装（`docs/40_decisions.md` §21 コミット3a）では、
**既存の作法に合わせて「認可は各ページの冒頭で行う。layout には置かない」と決めている**
（`/manager/layout.tsx`・`/dashboard/layout.tsx` が metadata 専用の pass-through であるため）。
**どちらに寄せるかは未判断。** 集約するならこの診断記録の指摘を採る、
個別実装を正とするなら §2-7 の「検討する」を取り下げる、という選択になる。

---

## 5. 次回予定

**2026-10-25（日）** に**第3回**定期診断を実施する（§1 の変更後のレビュー日＝毎月25日）。
