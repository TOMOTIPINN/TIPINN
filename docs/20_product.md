...
| `/rating?salon=&staff=&review=` | **有償の評価スタンプ選択**。tier 選択 → `/api/checkout` → Stripe へ。**購入には「購入可能な感想」の `review` が必須**（本人・当該サロン/スタッフ宛て・`share_scope='everyone'`・`rating>=3`・未購入）。入口は `/review/complete` の「評価スタンプを送る」のみで、URL 直打ちや条件外は 400/409 で弾く（§13）。ティア選択画面に「お名前とスタンプの種類が、担当スタッフとサロンに表示されます。」を表示 |
