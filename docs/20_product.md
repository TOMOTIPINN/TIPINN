...
| `/rating?salon=&staff=&review=` | **有償の評価スタンプ選択**。tier 選択 → `/api/checkout` → Stripe へ。**購入には「購入可能な感想」の `review` が必須**（本人・当該サロン/スタッフ宛て・未購入）。**`rating` も `share_scope` も条件に入らない**（§14 で `rating>=3` を撤廃・§19 で `share_scope='everyone'` を撤廃）。**低評価の感想にも `manager_only` の感想にも贈れる**（本文が本人に届かない分は、表示側で「名前・ティア・rating」を届けることで解いた・§19）。入口は `/review/complete` の「評価スタンプを送る」のみで、URL 直打ちや条件外は 400/409 で弾く（§13・§14）。ティア選択画面に「お名前とスタンプの種類が、担当スタッフとサロンに表示されます。」を表示 |
| `/rating/complete?session_id=` | 購入完了。**DB書き込みなし**（記録は webhook 側・二重記録防止） |
| `/mypage` | サロンごとのスタンプカード（円形ロゴ＋スタンプリング＋次の特典までの進捗）。移行 delta を合算。特典の3状態表示。**感想・評価スタンプへの補助導線**（受付期間内のみ・LINE リマインドの置き換えではなく補助・§15） |
...
| ルート | 役割 |
|---|---|
| `/manager/inbox` | 店長Inbox（お客様が選んだ公開範囲を表示・**店長は変更しない**・§16。新着順に50件ずつ「もっと見る」／`?scope=` で 全部・店長のみ・お店のスタッフに を絞り込み・件数はサロン全体・§17）。**顧客名とティアの両方が出る**（§20 決定1・`rating_purchases` から `review_id, tier` だけを引く。ティアは公開範囲と同じ褪せグレーで、**高ティアを強調しない**）。**各行は `/staff/received/[reviewId]?from=inbox` へのリンク**（§20 決定2。`?from=inbox` は詳細画面の戻り先を `/manager/inbox` にするための印で、**権限判定には使わない**）。**ティアが付くのは新しい購入だけ**（過去21件は `review_id` が null・§13 決定6）。**金額は出さない（原則5）** |
...
| `/owner` | **未実装（§21・2026年10月末目標）。** オーナーが自社の複数店舗を**店舗を切り替えて**見る層。中身は `/manager/inbox` と `/dashboard` の `salonId` 引数版。**5店舗を横に並べた全スタッフ一覧は作らない**（店舗間ランキングを生まないため）。**店舗単位の数字の比較は出す。** 閲覧記録（店長がいつ・どの感想を読んだか）は**作らない** |

**2タブ構成：**
- **日次ビュー** — 今の状態。先行指標4枚＝**感想**／**評価スタンプ**（件数は別カード・**合算しない**・§18 B）／店舗合計¥／VIP累計。さらに
  ティア別の内訳（件数＋**売上**・§18）／スタッフ別（**感想の4段階別内訳**・ティア絵文字内訳・
  リアルボイス・前期間比）／最近の評価（**日付（M/D HH:mm）・顧客名・スタッフ名・ティア**・§20 決定3。**5件ずつの「もっと見る」**＝`?take=`・上限100・§20 決定4。**スマホ幅（≤520px）は2段**: 1段目「日付・ティア」／2段目「○○様 → スタッフ名」）／
  VIP のお客様（**顧客名＋無償スタンプ数**。有償ティアではない）。
  **個人¥なし・スコアなし・件数順に並べ替えない**（§18）
  - **§20（2026-09-19 決定・2026-09-23 実装・本番確認済み）**: 「最近の評価」の顧客名・日付・「もっと見る」と、
    `/manager/inbox` のティアバッジ・詳細へのリンク。**migration 無し**（既存列のみ）
- **HR月次ビュー（echo flow）** — トレンド。echo flow の推移・要ケアアラート・好調/要ケア詳細。
  **金額は出さない**（§18 D。数字はすべて直近3ヶ月で揃える）
- 前期間比には**実際の日付範囲**を添える（前期間＝直前の同じ長さの窓・§18 E）
