...
| ルート | 役割 |
|---|---|
| `/manager/inbox` | 店長Inbox（お客様が選んだ公開範囲を表示・**店長は変更しない**・§16。新着順に50件ずつ「もっと見る」／`?scope=` で 全部・店長のみ・お店のスタッフに を絞り込み・件数はサロン全体・§17）。**顧客名は出る／ティアは出ない**（`rating_purchases` の参照0件）。⚠️ **§20 で「行にティアバッジ」と「行から `/staff/received/[reviewId]` への導線」を足すと決定（2026-09-19・実装は未着手）。金額は引き続き出さない（原則5）** |
...
- **日次ビュー** — 今の状態。先行指標4枚＝**感想**／**評価スタンプ**（件数は別カード・**合算しない**・§18 B）／店舗合計¥／VIP累計。さらに
  ...
  リアルボイス・前期間比）／最近の評価（**時刻・スタッフ名・ティア。顧客名は出さない**）／
  VIP のお客様（**顧客名＋無償スタンプ数**。有償ティアではない）。
  **個人¥なし・スコアなし・件数順に並べ替えない**（§18）
  - ⚠️ **§20（2026-09-19 決定・実装は未着手）**: 「最近の評価」に**顧客名**を足し
    （時刻・顧客名・スタッフ名・ティア）、**5件ずつ遡る「もっと見る」**を足す
