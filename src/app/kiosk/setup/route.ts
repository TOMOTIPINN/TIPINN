 ...
 * 以後その端末は LINE ログイン無しで /staff/visit を使える（salon スコープは cookie 内の salon_id 固定）。
 *
 * 照合: salons を id で引き、device_token が一致し非null のときだけ cookie を発行する（推測URLを弾く）。
 *   一致 → echo_device cookie（署名JWT・httpOnly・1年）→ /staff/visit へ。
 *   不一致/未発行/不存在 → /staff/visit?device=error（キオスクを LINE ログインに飛ばさない）。
 *
...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  const fail = () =>
    NextResponse.redirect(new URL("/staff/visit?device=error", baseUrl), {
      status: 303,
    });
  ...
  const res = NextResponse.redirect(new URL("/staff/visit", baseUrl), {
    status: 303,
  });
