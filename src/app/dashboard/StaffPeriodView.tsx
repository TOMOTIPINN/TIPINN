 ...
 *
 * ※ 集計期間は上部の PeriodSelector（URL 由来）で決まり、cur/prev はその期間で集計済み＝件数・
 *   ティア内訳はそのまま期間連動する（このビューは期間選択 UI を持たない）。対象スタッフ選択（下記）は
 *   props 上の純client フィルタ。¥（per-staff revenue）は原則5・§12 のため出さない（集計層で 0）。
 */
