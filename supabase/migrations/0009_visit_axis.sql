-- 0009 — 来店スタンプ軸（Visit axis / Phase 7）
-- 準拠: CLAUDE.md §2 絶対原則 / §4 データモデル・RLS / 0004（感想RPCの advisory lock + JST日付判定を流用）
--
-- 目的:
--   感想軸（earned_stamps・3回固定）とは別に「来店軸」を追加する。
--   ・サロンごとに可変ハードル visit_cycle_size（既定20・10〜20）。デフォルトOFF（visit_axis_enabled）。
--   ・来店カウントは累計（リセットしない）。発火は visit_cycle_size の倍数ごと（20,40,60…）。
--   ・特典は感想軸と共通の rewards（最大2件）。来店到達でも同じ rewards が発火（DB側は特典を持たない）。
--   ・不正防止＝1日1回・JST(Asia/Tokyo)日付ロック（unique制約 ＋ advisory lock）。
--   ・来店軸は無料・無決済。rating_purchases（有償）スキームには一切干渉しない。
--
-- 設計判断（確定事項）:
--   ・RPCは visit_axis_enabled を判定しない＝常に記録する。表示のON/OFFはUI側で判定
--     （OFF中も累計を貯めておき、後からONで自然に活きる）。
--   ・visits は単一テーブル・COUNT累計方式（カウンタ列やミラーテーブルは作らない）。
--
-- 方針（0006/0004 と同じ作法）:
--   ・列追加は add column if not exists ＋ drop constraint if exists → add constraint。
--   ・RLS は deny-by-default（0001）。visits はRLS有効・ポリシー作らない＝service_role のみ。
