...
| `docs/70_legal.md` | 法務の決定と依頼状況（回答済み／依頼中／未依頼／未確認） |

**矛盾したら `00` > `40` > `10`/`20`/`30`/`50` > `60` > `70` > `CLAUDE.md` の順で優先する。**

`60` が設計文書より下・`CLAUDE.md` より上なのは、**60 は起きた事実の記録であり設計判断ではない**ため。
設計文書と矛盾する場合は設計文書が正。ただし**実際に観測された事実は作業指示より重い**ので `CLAUDE.md` より上に置く。

`70` は**外部（弁護士など）の判断の台帳**。当社の設計判断ではないので設計文書より下だが、
`60`（起きた事実）とは性格が違うため番号を分けている。**法務の回答は `70_legal.md` を正とする。**
