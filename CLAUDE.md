...
- **`public` に `create table` すると RLS が自動で有効化される。** イベントトリガー `ensure_rls`（関数 `public.rls_auto_enable`・owner=postgres）が `ddl_command_end` で動いており、テーブルを作った瞬間に `enable row level security` が実行される。**ポリシーを書かなければ完全deny＝REST から一切読めない**（`supabaseAdmin`＝service_role はバイパスするので書き込み・RPC は通り、気づきにくい）。トリガーは `RAISE LOG` のみで画面には何も出ない。→ **新テーブルを作るときは、同じ migration 内でポリシーまで書き切るか、「完全deny のままにする」ことを意図的に選ぶこと**（`0025 reward_redemptions` は後者＝ポリシー0本の完全deny・読む導線は 0030 等の security definer RPC 経由）。2026-07-17 の監査で発見。migrations に記録が無かったため `0031` として事後記録（出所は Supabase Security Advisor の Fix と推定・owner=postgres）。
...
- コピーは温かく急かさない（「来店当日中ならおうちでもOK・あとで送る」を堂々と用意）
  - ⚠️ **感想は「来店当日(JST)」に限る**（migration 0032 の来店裏付けチェック＝farming 対策）。当日中なら自宅からでも送れるが、**翌日以降はゲートで送信不可**＝RPC が `no_visit_today` を raise → `/api/reviews` が 409 `{error:"no_visit_today"}` → `ReviewForm` が「本日のご来店が確認できませんでした。ご来店当日にお試しください。」を表示。「おうちでもOK」は"当日中の自宅送信OK"の意で、"いつでもOK"ではない。
